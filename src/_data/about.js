module.exports = {
    topTitle: "Om oss",
    title: "Om Rasbo Hundsalong",
    text: [
        "På Rasbo Hundsalong står hundens välmående i fokus. Med stor omsorg och ett lugnt bemötande ser vi till att varje hund får den vård och omsorg den behöver. Oavsett om det gäller klippning, bad eller detaljerad pälsvård, anpassar vi oss efter hundens behov och din önskan. Vår salong i Rasbo är en plats där hundar får känna sig trygga och bli omhändertagna med kärlek och professionalism. Välkommen!",
        "In consequat tincidunt turpis sit amet imperdiet. Praesent Class officelan nonatoureanor mauris laoreet, iaculis libero quis. Curabitur et tempus eri consequat tincidunt turpis sit amet imperdiet. Praesent nonatourean olei aptent taciti sociosqu ad litora torquent per.",
    ],
    quote: "",
    quoteName: "Angelika",
    quotePosition: "",
    image: "/assets/images/dog-grooming3.jpg",
    imageMobile: "/assets/images/dog-grooming3-m.webp",
    imageSecondary: "/assets/images/dog-grooming4.jpg",
    imageSecondaryMobile: "/assets/images/dog-grooming4-m.webp",
};
